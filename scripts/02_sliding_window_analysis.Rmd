...
yearfit1 <- predict(yearmod1, newdata = yeardf1, type = "link", se = TRUE)
yearfit1 <- as.data.frame(yearfit1)
yeardf1$preds<-plogis(yearfit1$fit)
#add variables for 95% confidence intervals 
yeardf1$minse<-plogis(yearfit1$fit-1.96*(yearfit1$se.fit))
yeardf1$maxse<-plogis(yearfit1$fit+1.96*(yearfit1$se.fit))
...
fptfit <- predict(modtmin49_276, newdata = fptplotdf, type = "link", se = TRUE)
fptfit <- as.data.frame(fptfit)
fptplotdf$preds<-plogis(fptfit$fit)
#add variables for 95% confidence intervals 
fptplotdf$minse<-plogis(fptfit$fit-1.96*(fptfit$se.fit))
fptplotdf$maxse<-plogis(fptfit$fit+1.96*(fptfit$se.fit))
...
tminplotgen <- ggplot(fptplotdf,aes(x=w_49_276,y=preds))+
  ...
  theme_classic()+
  xlab("Mean Daily Minimum Temperature (\u00B0C) Between 1st June and 19th July")+
  ylab("Probability of Two Flight Periods")+
...
#add variables for 95% confidence interval
tempplotdf1$sel<-mintemppreds$fit-(1.96*mintemppreds$se.fit)
tempplotdf1$seu<-mintemppreds$fit+(1.96*mintemppreds$se.fit)
tempplotdf1$var <- "min"
