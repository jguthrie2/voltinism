...
##best model - highest R2 - 0.28
#w_60_279.5
modtmax60_279.5<-glm(Gen~w_60_279.5,data=fptmax,family=binomial)
summary(modtmax60_279.5)
```

actually day 179.5; day 179 is 30th day and day 180 is 31st day;

therefore 179-29 = 150, start day is 150
...
##best model - r2 0.35
#w_57_279
#window length 57 days
#centred on day 279
modtmean57_279<-glm(Gen~w_57_279,data=fptmean,family=binomial)
summary(modtmean57_279)
```

day 279 is actually day 179
day 179 is 29th day - there are 28 days prior to this and 28 days after
start day = 179-28 = 151
end day = 179 + 28 = 207
...
### first test how the number of years of data and the timing of the data (old or recent) affects the model estimate
...
#also add data for mean and maximum temperature in their respective best windows
tmaxmod1 <- lm(w_60_279.5 ~ Year, data = tmaxwindows)
tempplotdf2 = data.frame(Year=rep(seq(from = 1968, to = 2023,by=0.1),length=1000))
maxtemppreds<-predict(tmaxmod1,newdata=tempplotdf2, se.fit=TRUE,type="response")
maxtemppreds<-as.data.frame(maxtemppreds)
tempplotdf2$preds<-maxtemppreds$fit
#add variables for confidence intervals - 2 standard errors
tempplotdf2$sel<-maxtemppreds$fit-(2*maxtemppreds$se.fit)
tempplotdf2$seu<-maxtemppreds$fit+(2*maxtemppreds$se.fit)
tempplotdf2$var <- "max"

tmeanmod1 <- lm(w_57_279 ~ Year, data = tmeanwindows)
tempplotdf3 = data.frame(Year=rep(seq(from = 1968, to = 2023,by=0.1),length=1000))
meantemppreds<-predict(tmeanmod1,newdata=tempplotdf3, se.fit=TRUE,type="response")
meantemppreds<-as.data.frame(meantemppreds)
tempplotdf3$preds<-meantemppreds$fit
#add variables for confidence intervals - 2 standard errors
tempplotdf3$sel<-meantemppreds$fit-(2*meantemppreds$se.fit)
tempplotdf3$seu<-meantemppreds$fit+(2*meantemppreds$se.fit)
tempplotdf3$var <- "mean"

alltempplotdf <- rbind(tempplotdf1, tempplotdf2, tempplotdf3)

tempplot <- ggplot(alltempplotdf,aes(x=Year,y=preds, color = var))+
  geom_ribbon(aes(ymin = sel, ymax =  seu, fill = var),alpha=0.2) +
  geom_line()+
  geom_point(data=tminwindows,aes(x=Year,y=w_49_276), color = "blue")+
  geom_point(data=tmaxwindows,aes(x=Year,y=w_60_279.5), color = "red")+
  geom_point(data=tmeanwindows,aes(x=Year,y=w_57_279), color = "green")+
  theme_classic()+
  ylab("Temperature in Critical Window")
tempplot

ggsave(filename = "outputs/figures/all_temp_year_plot.png",
       plot = tempplot,
       device = "png",
       width = 18,
       height = 12,
...
##plot temperature with number of generations
...
tempgenplot <- ggplot(tempplotdf1,aes(x=Year,y=preds))+
  ...
  geom_line()+
  geom_point(data=fptminsub,aes(x=Year,y=w_49_276, color = Gen))+
  theme_classic()+
  scale_color_manual(name = "Number of\nFlight Periods", labels = c("1", "2"), values = c("#7ad151", "#440154"))+
  ylab("Temperature in Critical Window")
tempgenplot

ggsave(filename = "outputs/figures/temp_year_fp_plot.png",
       plot = tempgenplot,
       device = "png",
       width = 18,
       height = 12,
...
## test whether temperature in the critical window explains occurrence of a second flight period better than time

```{r}
#extract residuals from previous model of temperature against year
yearresid <- tymod1$residuals
#add to data frame
tydf$yearresid <- yearresid


#model containing both temperature and residuals of year as explanatory variables
tyrmod1 <- glm(Gen ~ w_49_276 + yearresid, data = tydf, family = binomial)
summary(tyrmod1)

#model with only temperature
tyrmod2 <- glm(Gen ~ w_49_276, data = tydf, family = binomial)
summary(tyrmod2)

#model with only year residuals
tyrmod3 <- glm(Gen ~ yearresid, data = tydf, family = binomial)
summary(tyrmod3)


##compare models with AIC
AIC(tyrmod1, tyrmod2, tyrmod3)


###model with both explanatory variables is best
```
